...
class DenseGeneral(nn.Module):
    ...
    def __init__(
        ...
        if use_bias:
            self.bias = nn.Parameter(torch.empty(self.bias_shape, **factory_kwargs))
        else:
            self.register_parameter("bias", None)  # Correct way to register no bias

        self.reset_parameters()  # Initialize parameters
        self._loaded: bool = False  # Flag to track if weights were explicitly loaded
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        # compute_dtype = self.dtype if self.dtype is not None else inputs.dtype
