...
def decode(
    ...
    """
    print("In decode method, audio_codes shape:", audio_codes.shape)
    print("In decode method, audio_codes type:", type(audio_codes))

    print("Checking frame count...")
    if len(audio_codes) != 1:
        raise ValueError(f"Expected one frame, got {len(audio_codes)}")

    print("Converting codes to audio values...")
    try:
...
def codebook_to_audio(generated_codes: torch.Tensor, model, delay_pattern, B=1, T=2600, C=9):
    ...
    print("Padded codebook shape:", generated_codes.shape)
    ...
    print("Reverted codebook:", reverted_codebook)
    codebook = reverted_codebook.transpose(1, 2)
    print("Reverted codebook shape:", codebook.shape)
